...
pub fn checked_mul_div(n0: u128, n1: u128, d: u128) -> Result<u128, OrcaError> {
    checked_mul_div_round_up_if(n0, n1, d, false)
}

pub fn checked_mul_div_round_up(n0: u128, n1: u128, d: u128) -> Result<u128, OrcaError> {
...
pub fn checked_mul_div_round_up_if(
    ...
    d: u128,
    round_up: bool,
) -> Result<u128, OrcaError> {
    if d == 0 {
        return Err(OrcaError::DivideByZero);
    }

    let p = n0.checked_mul(n1).ok_or(OrcaError::MulDivOverflow)?;
    let n = p / d;

    Ok(if round_up && p % d > 0 { n + 1 } else { n })
}

pub fn checked_mul_shift_right(n0: u128, n1: u128) -> Result<u64, OrcaError> {
...
pub fn checked_mul_shift_right_round_up_if(
    ...
    let p = n0
        .checked_mul(n1)
        .ok_or(OrcaError::MultiplicationShiftRightOverflow)?;

    let result = (p >> Q64_RESOLUTION) as u64;

    let should_round = round_up && (p & Q64_MASK > 0);
    if should_round && result == u64::MAX {
        return Err(OrcaError::MultiplicationOverflow);
    }

    Ok(if should_round { result + 1 } else { result })
}

pub fn div_round_up(n: u128, d: u128) -> Result<u128, OrcaError> {
    div_round_up_if(n, d, true)
}

pub fn div_round_up_if(n: u128, d: u128, round_up: bool) -> Result<u128, OrcaError> {
    if d == 0 {
        return Err(OrcaError::DivideByZero);
    }
